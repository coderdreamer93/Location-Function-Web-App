  ...
  return (
    <aside className="relative w-full border-r border-gray-200 h-[calc(100vh-3.5rem)] overflow-y-auto p-3">
      ...
      <div className="pt-3">
        <button
          onClick={applyFilters}
          className="w-full newPrimaryBg text-white text-sm py-2 rounded-lg font-medium hover:newPrimaryBg transition"
        >
          Apply
        </button>
      </div>
      ...
      <ul className="ml-0 pl-0 space-y-1">
        {options.map((item) => (
          ...
          >
            {item}
            {isSelected(item) && (
              <span className="newPrimaryColor text-[14px] absolute right-12">
                <RightTick />
              </span>
            )}
          </li>
